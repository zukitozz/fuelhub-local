    ...
    const cierres = await executeQuery<IReporteDeclaracionMensual[]>(
        ...
        `
            select
                ...
                i.descripcion,
                case when tipo_comprobante = '07' then CAST(ISNULL(i.valor_venta, 0) as decimal(18,2)) * -1 else CAST(ISNULL(i.valor_venta, 0) as decimal(18,2)) end as valor_venta,
                case when tipo_comprobante = '07' then CAST(ISNULL(i.igv_venta, 0) as decimal(18,2)) * -1 else CAST(ISNULL(i.igv_venta, 0) as decimal(18,2)) end as igv_venta,
                c.fecha_documento_afectado, c.numeracion_documento_afectado,
                c.tipo_moneda,
