    ...
    const cierres = await executeQuery<IReporteDeclaracionMensual[]>(
        ...
        `
            select
                ...
                c.tipo_comprobante, c.numeracion_comprobante,
                r.tipo_documento, r.numero_documento, r.razon_social,
                i.descripcion, CAST(ISNULL(i.cantidad_venta, 0) as decimal(18,3)) as cantidad_venta,
                CAST(ISNULL(i.valor_venta, 0) as decimal(18,2)) as valor_venta,
                CAST(ISNULL(i.igv_venta, 0) as decimal(18,2)) as igv_venta,
                c.fecha_documento_afectado, c.tipo_documento_afectado, c.numeracion_documento_afectado,
                CAST(ISNULL(i.precio, 0) as decimal(18,2)) as precio, c.tipo_moneda,
                CAST(ISNULL(c.pago_efectivo, 0) as decimal(18,2)) as pago_efectivo,
                CAST(ISNULL(c.pago_tarjeta, 0) as decimal(18,2)) as pago_tarjeta,
                CAST(ISNULL(c.pago_yape, 0) as decimal(18,2)) as pago_yape,
                c.ruc
            from Comprobantes c
            inner join Receptores r on c.ReceptorId = r.id
            inner join Items i on i.ComprobanteId = c.id
            ...
            order by c.fecha_emision, c.id, i.id;
        `
    );    
    return cierres;
}
